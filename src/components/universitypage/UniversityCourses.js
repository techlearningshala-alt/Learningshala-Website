...
const UniversityCourses = ({ id, title,universityData, props }) => {
  const [isOpen, setIsOpen] = useState(false);


  return (
    <><div id={id} className="content_card">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4"><UniversityContentHeading className='!m-0' title={title} />
        <Link href="/tools/roi-calculator"> <SpanButton title="ROI Calculator" icon={<Calculator className="w-4 h-4" />} /></Link>
      </div>
      <div className="grid grid-cols-12 gap-6 mb-6">
        {props?.coursesList?.slice(0, 4).map((data, index) => (
              ...
              <div>
                ...
                <p className="flex justify-between items-center bg-gray-100 py-3 px-4 rounded-xl">
                  ...
                  <Link
                    ...
                    href={`/university/${universityData.university_slug}/${data.course_slug}`}
                  >
                    {" "}
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        <div onClick={() => setIsOpen(true)}><PrimaryButton iconPos="left" icon={<Eye />} title={`View All ${props?.coursesList?.length} Courses`} /></div>
        ...
        <DrawerContent className="pt-4">
          ...
          <ScrollArea className="h-[600px]"> <div className="container">
            <div className="grid grid-cols-12 gap-6 my-6 pb-20">
              {props?.coursesList?.map((data, index) => (
